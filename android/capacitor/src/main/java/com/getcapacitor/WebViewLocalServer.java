    ...
    private WebResourceResponse handleProxyRequest(WebResourceRequest request, PathHandler handler) {
        ...
        try {
            ...
            if (request instanceof WebResourceRequestWithBody) {
                ...
                } else if (wwwb.getInterceptor().mNextFormRequestContents != null) {
            ...
            try {
                ...
                try {
                    Logger.error("Response error: ", fex);
                    InputStream errStr = conn.getErrorStream();
                    responseStream = errStr;
                } catch (Exception e) {
                    Logger.error("Error stream: ", e);
                }
            }
            if (responseStream == null)
                responseStream = new ByteArrayInputStream((responseCode+": "+responseReason).getBytes(StandardCharsets.UTF_8));
